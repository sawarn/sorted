...
object Categorizer {
    ...
    private fun merchantRule(key: String): CategoryResult? {
        return when {
            ...
            key.contains("IPRUMF") -> rule("ICICI Prudential Mutual Fund", "Mutual Fund", "Investment", TransactionType.INVESTMENT)
            key.contains("HDFCMF") -> rule("HDFC Mutual Fund", "Mutual Fund", "Investment", TransactionType.INVESTMENT)
            key.contains("QNTAMC") || key.contains("QUANT MUTUAL FUND") -> rule("Quant Mutual Fund", "Mutual Fund", "Investment", TransactionType.INVESTMENT)
            key.contains("EDLAMC") || key.contains("EDELWEISS ASSET MANAGEMENT") -> rule("Edelweiss Mutual Fund", "Mutual Fund", "Investment", TransactionType.INVESTMENT)
            key.contains("MOAMCL") || key.contains("MOTILAL OSWAL MF") -> rule("Motilal Oswal Mutual Fund", "Mutual Fund", "Investment", TransactionType.INVESTMENT)
